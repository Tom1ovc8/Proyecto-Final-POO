...
La clase `Extracts` proporciona una serie de métodos estáticos diseñados para consultar, exportar, importar y reconstruir información del sistema de inventario. Sirve como puente entre la lógica del sistema y los archivos externos `.json`, permitiendo tanto respaldar como restaurar datos completos o parciales.
...
```
Estos métodos devuelven listas de diccionarios con la información de los diferentes objetos del sistema: movimientos, facturas, registros de inventario, clientes y proveedores. Para lograr esto, se invoca el método `to_dict()` sobre cada instancia.
...
```
Este método genérico escribe cualquier lista de datos en un archivo JSON. Si ocurre un error en el proceso, se lanza una excepción detallada. Los métodos específicos de exportación llaman a este:
...
```
También está `export_full_system`, que guarda todos los elementos clave en un solo archivo:
...
```
Este método carga productos desde un archivo y los convierte en objetos `Product` usando:
...
```
Este fragmento interpreta correctamente el estado del producto según sea por fecha de vencimiento o condición física.

Reconstruccion de objetos desde diccionarios:
        ...
        for movement_data in data["record"]:
            ...
            if code in system.records:
                try:
                    ...
                    raise ValueError(f"Couldn't load movement for {code}: {e}")

    return stock
```
Este método no solo reconstruye el objeto `Stock`, sino también su historial de movimientos, evitando duplicaciones.
...
def dict_to_location(data):
    return Location(data["aisle"], data["shelf"])
...
def dict_to_bill(data, system):     
    ...
    return bill
```
Este método reconstruye una factura con todos sus componentes: entidad, productos, precios, cantidades y forma de pago.
...
```
Y finalmente, la carga total del sistema:
...
def load_full_backup(path, system):
    ...
    for supplier in data["suppliers"]:
        existing = next(
            (x for x in system.suppliers.values() 
             if x.name == supplier["name"] and x.contact_number == supplier["contact_number"]),
            None
        )
        if not existing:
    ...
    for record in data["records"]:
        ...
        if product_code in system.records:
            ...
            existing_keys = {
                (m.product._code, m.amount, m.actor._id, m.date.isoformat())
                for m in old_stock._record
            }
            for m in new_stock._record:
                key = (m.product._code, m.amount, m.actor._id, m.date.isoformat())
    ...
    for bill_data in data.get("bills", []):
        ...
        for m in system.movements:
            for item in bill.items:
                if (m.product._code == item.product._code and
                    m.amount == item.quantity and
                    m.actor._id == bill.entity._id):
                    m._bill_id = bill._bill_id
                    break

    print(f"Backup loaded successfully from {path}")
```
Este proceso es cuidadoso y robusto. Verifica duplicados, asocia correctamente productos con actores y movimientos, y asegura que las facturas se vinculen con los movimientos correspondientes.
